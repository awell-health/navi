...
class ThemeSyncer {
    ...
    if (!this.options.dryRun) {
      try {
        if (process.env.EDGE_CONFIG_URL) {
          ...
          this.edgeConfigClient = createClient(process.env.EDGE_CONFIG_URL);
  ...
  async syncAll(): Promise<SyncSummary> {
    ...
    try {
      ...
      console.log(`📥 Fetched branding for ${orgBrandings.length} organizations`);
      ...
      for (const orgBranding of orgBrandings) {
        try {
          ...
          results.push(result);
          console.error(`❌ ${orgBranding.orgId}: Unexpected error - ${result.error}`);
        }
      }
  ...
  private async fetchAllOrgBranding(): Promise<OrgBrandingResponse[]> {
    const apiUrl = this.options.sourceApi || process.env.BRANDING_API_URL;
    
    if (!apiUrl) {
      throw new Error('No API URL specified. Set BRANDING_API_URL or use --source-api');
    }
    ...
    try {
      const response = await fetch(apiUrl, {
        headers: {
          'Authorization': `Bearer ${process.env.BRANDING_API_TOKEN}`,
...
class MockEdgeConfigClient {
  async set(key: string, value: any): Promise<void> {
    console.log(`[MOCK] Set ${key}:`, JSON.stringify(value, null, 2).substring(0, 100) + '...');
  }
}
...
Examples:
  ...
  try {
    ...
    if (options.syncAll) {
      ...
      console.log('ℹ️  Note: Single org sync requires manual branding data input');
    }
