...
class PerformanceTester {
  ...
  async generateTestTokens(): Promise<{ [orgId: string]: string }> {
    ...
    try {
      ...
      for (const line of lines) {
        if (line.includes('http://localhost:3000/magic/')) {
          // Extract org type and token from the output
          if (line.includes('SUNRISE HEALTH')) {
            const url = line.match(/http:\/\/localhost:3000\/magic\/[^\s]+/)?.[0];
  ...
  async measurePerformance(url: string): Promise<{ fcp: number; lcp: number; cls: number }> {
  ...
  private checkBudget(metrics: { fcp: number; lcp: number; cls: number }): boolean {
    return (
  ...
  logResults(results: PerformanceResult[]): void {
    ...
    console.log('Org ID'.padEnd(15) + 'FCP'.padEnd(8) + 'LCP'.padEnd(8) + 'CLS'.padEnd(8) + 'Status');
    console.log('─'.repeat(80));
    ...
    console.log('─'.repeat(80));
    console.log(`Budget: FCP < ${PERFORMANCE_BUDGET.fcp}ms, LCP < ${PERFORMANCE_BUDGET.lcp}ms, CLS < ${PERFORMANCE_BUDGET.cls}`);
    ...
    if (passed === total) {
      ...
      console.log(`❌ ${total - passed}/${total} themes failed performance budget`);
    }
  }
}
