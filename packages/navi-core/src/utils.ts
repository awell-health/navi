import type { NaviConfig, JWTPayload, AuthResult, BrandingConfig } from './types';
...
export class AuthClient {
  ...
  async authenticate(pathwayId: string): Promise<AuthResult> {
    ...
    const mockPayload: JWTPayload = {
      ...
      environment: this.config.publishableKey.startsWith('pk_live_') ? 'production' : 'test',
...
// Branding utilities
export function generateCSSCustomProperties(branding: BrandingConfig): Record<string, string> {
  return {
