...
export async function getPathwayActivities(
  ...
  try {
    ...
    if (data.pathwayActivities?.activities) {
      ...
      console.log(
        `✅ Found ${stakeholderActivities.length} activities for stakeholder ${stakeholderId}`
      );

      if (stakeholderActivities.length > 0) {
        console.log(
          "🎯 Filtered activities for stakeholder:",
          stakeholderActivities.map((a: Activity) => ({
            id: a.id,
            careflow_id: a.careflow_id,
            status: a.status,
            has_form: a.inputs?.__typename === "FormActivityInput",
          }))
        );
      }
