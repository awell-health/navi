...
export const getBrandingAction = cache(async (): Promise<BrandingResult> => {
  ...
  try {
    ...
    if (sessionCookie) {
      ...
      if (session) {
        console.log(
          "✅ Server Action: Session found, fetching branding for org:",
          session.orgId
        );
        ...
        console.log(
          "⚠️  Server Action: Session not found in store, using default branding"
        );
      }
    } else {
      console.log(
        "⚠️  Server Action: No session cookie found, using default branding"
      );
    }
