...
describe('Favicon Utilities', () => {
  ...
  it('should handle URLs with query params and fragments', () => {
    expect(getFaviconType('https://cdn.example.com/favicon.png?v=1#hash')).toBe('image/png');
    expect(getFaviconType('/assets/favicon.svg?cache=123')).toBe('image/svg+xml');
  });
  ...
  it('should generate complete favicon HTML', () => {
    ...
    expect(html).toBe('<link rel="icon" type="image/png" href="https://example.com/favicon.png">');
  });
  
  it('should generate default favicon HTML when no branding', () => {
    const html = generateFaviconHTML(null);
    expect(html).toBe('<link rel="icon" type="image/png" href="/favicon-16x16.png">');
  });
}); 