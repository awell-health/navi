...
Emitted in real-time when data values change within an activity (form field inputs, checklist items). **PHI-safe**: This event does not contain actual field values or form data.
...
interface ActivityDataChangeEvent extends ActivityEvent<{
  ...
  data: {
    ...
    fieldType?: string      // Type of field for UI logic
    hasValue: boolean       // Whether field has any value
    isRequired?: boolean    // Whether field is required
    validationState?: string // Validation status (optional)
  }
  timestamp: number
}
```
