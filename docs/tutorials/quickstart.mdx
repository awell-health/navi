---
title: Quickstart
description: Embed Navi in 5 minutes (React or JavaScript).
---
...
- In CareOps, create a publishable key (pk\_...). You’ll use it in your app.
...
- In CareOps, create a publishable key (pk\_...).
...
- Replace `cfdef_12345` with your actual careflowDefinitionId from CareOps.
- For React, prefer storing the publishable key in `NEXT_PUBLIC_NAVI_PK` (or similar) and passing it to `NaviProvider`.
- You can customize styling via `branding` (primary colors, fonts, etc.) when calling `NaviEmbed` or `navi.render`.
