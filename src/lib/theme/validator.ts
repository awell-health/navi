...
const cssUnitSchema = z.string().regex(
...
export const orgBrandingSchema = z.object({
  orgId: z.string().min(1).max(50).regex(/^[a-zA-Z0-9\-_]+$/, 'Invalid orgId format'),
...
export function validateBranding(data: unknown): { success: true; data: ValidatedBranding } | { success: false; error: string } {
  try {
...
export function validateOrgBranding(data: unknown): { success: true; data: ValidatedOrgBranding } | { success: false; error: string } {
  try {
