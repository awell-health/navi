...
describe('Magic Link Route - Branding Failure Scenarios', () => {
  ...
  beforeEach(async () => {
    ...
    mockGenerateInlineThemeStyle.mockReturnValue('<style>/* default */</style>');
    mockGenerateFaviconHTML.mockReturnValue('<link rel="icon" href="/favicon.ico">');
  ...
  describe('Branding Service Failures', () => {
    ...
    it('should handle malformed branding data gracefully', async () => {
      ...
      expect(response.status).toBe(200);
      expect(mockGenerateInlineThemeStyle).toHaveBeenCalledWith(malformedBranding);
    });
  });
